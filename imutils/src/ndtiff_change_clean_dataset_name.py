...
def process_dataset(src_path, dst_path, dataset_name=None, verbose=False):
    ...
    try:
        ...
        # Track processed and skipped images
        processed_images = 0
        skipped_images = 0
            ...
            for t in range(timepoints):
                for c in range(channels):
                    for z in range(z_slices):
                        ...
                        if reader_obj.has_image(position=p, time=t, channel=c, z=z):
                            ...
                            if verbose:
                                print(f"Skipping missing image at p{p} t{t} c{c} z{z}")
                            skipped_images += 1
        ...
        print(f"Images processed: {processed_images}, Images skipped: {skipped_images}")
        return True
...
def main():
    ...
    parser.add_argument("--src_folder", required=True,
                        help="Source folder containing NDTIFF datasets or parent folder with dataset subfolders")
