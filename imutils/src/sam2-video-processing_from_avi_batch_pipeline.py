'''
- **CUDA Check**: The script checks if a GPU (CUDA) is available and uses it for processing if possible.

- **DLC CSV Parsing**: It reads a DeepLabCut CSV file and converts it into a `pandas` DataFrame with a MultiIndex for better organization.

- **Coordinate Extraction**: The script selects coordinates with the highest likelihood (or randomly in case of a tie) for body parts and uses them to create segmentation masks.

- **Video Frame Extraction**: It extracts frames from the input video in batches and organizes them into folders.

- **Mask Generation**: The script uses a SAM2 model to generate segmentation masks for each frame, propagating them forward and backward through the video.

- **Mask Processing**: The masks are processed into binary format and saved into a TIFF file.

- **Cleanup**: Temporary files are deleted after processing to avoid clutter.

The script is intended to work with video data, extract keypoints from DLC output, and generate corresponding binary masks that can be saved for further analysis or visualization.
'''
    ...
    for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state, reverse=False):
        for i, out_obj_id in enumerate(out_obj_ids):
            ...
            if len(mask.shape) == 2:
                ...
                print(
                    f"Binary mask for frame {out_frame_idx} has unique values: {np.unique(binary_mask)} and shape: {binary_mask.shape}")
...
def main(args):
    ...
    parser.add_argument("--downsample_factor", type=float, default=0,
                        help="Use in case DLC was used on downsampled video (default: 0)")
    ...
    for batch_number, _ in batch_frame_count:
        ...
        print(
            f"Masklet generated for batch {batch_number}. Number of masks: {len(masks)}. Stored masks: {list(masks.keys())}")
